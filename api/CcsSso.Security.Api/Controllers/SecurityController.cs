...
namespace CcsSso.Security.Api.Controllers
{
  ...
  public class SecurityController : ControllerBase
  {
    ...
    [SwaggerOperation(Tags = new[] { "security" })]
    public async Task<IActionResult> Authorize(string scope, string response_type, string client_id, string redirect_uri, string prompt = null)
    {
      var url = await _securityService.GetAuthenticationEndPointAsync(scope, response_type, client_id, redirect_uri, prompt);
      return Redirect(url);
    }
    ...
    public async Task<TokenResponseInfo> Token(TokenRequestInfo tokenRequestInfo)
    {
      ...
      var idToken = await _securityService.GetRenewedTokenAsync(tokenRequestInfo, opbsValue, host);
