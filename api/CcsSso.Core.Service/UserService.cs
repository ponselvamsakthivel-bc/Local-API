...
namespace CcsSso.Service
{
  public class UserService : IUserService
  {
    ...
    {
      var user = await _dataContext.User.Include(u => u.UserGroupMemberships).ThenInclude(c => c.UserGroup).FirstOrDefaultAsync(u => u.UserName.ToLower() == userName.ToLower());
      if (user != null)
      {
        UserDetails userProfileDetails = new UserDetails()
        {
          ...
          UserGroups = user.UserGroupMemberships?.Select(ug => new UserGroup()
          {
            ...
            Group = ug.UserGroup?.UserGroupName
          }).ToList()
        };
        return userProfileDetails;
      }
